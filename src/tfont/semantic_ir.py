...
def _lock_sort_key(value: OntologyLockFingerprint) -> tuple[bytes, ...]:
    return (
        _utf16(value.lock_id),
        _utf16(value.ontology_id),
        _utf16(value.release),
        _utf16(value.content_digest),
        _utf16(value.term_namespace),
    )
...
def _release_sort_key(value: ProfileReleaseKey) -> tuple[bytes, bytes, bytes]:
    return (_utf16(value.corpus_id), _utf16(value.authored_profile_id), _utf16(value.profile_version))
...
def _identity_key_sort(value: IdentityKey) -> tuple[bytes, ...]:
    return tuple(_utf16(item) for item in (value.authority_system, value.external_entity_id, value.identity_strength))
...
def _dependency_records(profile: dict[str, Any]) -> tuple[tuple[str, str], ...]:
    ...
    for dependency in profile.get("dependencies", []):
        ...
        rows.append((dependency["dependency_id"], canonical_json_bytes(normalized).decode("utf-8")))
...
def _bundle_context(bundle: ValidatedSemanticBundle) -> tuple[str, ProfileReleaseKey, BundleVariantKey, ProfileReleaseSignature, BundleVariantIR]:
    ...
    participating_ids = _participating_lock_ids(bundle)
    lock_fingerprints = tuple(sorted((_lock(locks[lock_id]) for lock_id in participating_ids), key=_lock_sort_key))
    mapping_digests = tuple(sorted(bundle.mapping_semantic_digests, key=lambda item: _utf16(item[0])))
    ...
    for mapping_id, mapping in sorted(mappings.items(), key=lambda item: _utf16(item[0])):
        ...
        for projection in sorted(mapping.get("projections", []), key=lambda item: _utf16(item["projection_id"])):
            projection_reviews.append((mapping_id, projection["projection_id"], _review(projection["review"])))
...
def _candidate_ir(candidate: dict[str, Any], *, variant: BundleVariantKey, corpus_id: str, mapping_id: str, locks: dict[str, dict[str, Any]]) -> CandidateIR:
    return CandidateIR(
        variant=variant,
        corpus_id=corpus_id,
...
def _external_reference_ir(reference: dict[str, Any], *, variant: BundleVariantKey, corpus_id: str, mapping_id: str) -> ExternalReferenceIR:
...
def _target_ir(projection: dict[str, Any], *, variant: BundleVariantKey, corpus_id: str, mapping: dict[str, Any], lock: dict[str, Any]) -> TargetBindingIR:
...
def compile_semantic_ir(bundles: Iterable[ValidatedSemanticBundle]) -> CompiledSemanticIR:
    ...
    contexts: list[tuple[ValidatedSemanticBundle, str, ProfileReleaseKey, BundleVariantKey, ProfileReleaseSignature, BundleVariantIR]] = []
    ...
    for bundle, corpus_id, _release_key, variant, _signature, _variant_ir in contexts:
        ...
        for mapping_id, mapping in sorted(mappings.items(), key=lambda item: _utf16(item[0])):
            ...
            candidates = tuple(
                _candidate_ir(candidate, variant=variant, corpus_id=corpus_id, mapping_id=mapping_id, locks=locks)
                for candidate in sorted(mapping.get("ambiguous_candidates", []), key=lambda item: _utf16(item["candidate_id"]))
            )
            references = tuple(
                _external_reference_ir(reference, variant=variant, corpus_id=corpus_id, mapping_id=mapping_id)
                for reference in sorted(mapping.get("external_references", []), key=lambda item: _utf16(item["reference_id"]))
            )
            native_record = NativeRecordIR(
                ...
                projection_ids=_strings([item["projection_id"] for item in mapping.get("projections", [])]),
                candidate_ids=_strings([item["candidate_id"] for item in mapping.get("ambiguous_candidates", [])]),
                reference_ids=_strings([item["reference_id"] for item in mapping.get("external_references", [])]),
            ...
            for projection in mapping.get("projections", []):
                ...
                if projection["reference_kind"] == "semantic-pivot" and projection["query_role"] == "semantic-constraint":
                    key = SemanticKey(
                    ...
                    semantic_groups.setdefault(key, []).append(target_ir)
                elif projection["reference_kind"] == "authority-value" and projection["query_role"] == "authority-value-filter":
                    lock = locks[projection["ontology_lock"]]
                    key = AuthorityKey(
                        lock["ontology_id"],
                        projection["target"],
                    ...
                    authority_groups.setdefault(key, []).append(target_ir)

                cap_key = CapabilityKey(variant, projection["profile_id"], projection["capability_id"])
                ...
                for reference in references:
                    if reference.reference_kind == "entity-identity":
                        identity_groups.setdefault(
                            IdentityKey(reference.authority_system or "", reference.external, reference.identity_strength or ""),
                            [],
                        ).append(reference)
                    elif reference.reference_kind == "catalogue-identifier":
                        identifier_groups.setdefault(
                            IdentifierKey(reference.issuer_or_namespace or "", reference.external),
                            [],
                        ).append(reference)
    ...
    semantic_index = tuple(
        ...
        for key, rows in sorted(semantic_groups.items(), key=lambda item: _semantic_key_sort(item[0]))
    )
    authority_index = tuple(
        (key, tuple(sorted(rows, key=_binding_sort)))
        for key, rows in sorted(authority_groups.items(), key=lambda item: _authority_key_sort(item[0]))
    )
    identity_index = tuple(
        (key, tuple(sorted(rows, key=_reference_sort)))
        for key, rows in sorted(identity_groups.items(), key=lambda item: _identity_key_sort(item[0]))
    )
    identifier_index = tuple(
        (key, tuple(sorted(rows, key=_reference_sort)))
        for key, rows in sorted(identifier_groups.items(), key=lambda item: _identifier_key_sort(item[0]))
    )
